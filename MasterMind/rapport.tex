...
La fonction principale et auxiliaire sont toutes deux récursives, la récursion de la fonction principale permet l'accumulation des appels de construire\_aux en fonction de la taille (grâce à l'appel: construire\_aux( construire\_ListR (taille-1) listC) ). Quand à la fonction auxiliaire, celle-ci permet d'ajouter chacune des couleurs possibles sur tous les élements de la liste placée en paramètre ( (Rouge::h)::(Bleu::h) ...). Grâce à cette double récursion la création de la liste est très efficace malgré les 32768 cas à produire pour une combinaison de taille 5.
\subsection{Exemple:}
%
\par
...
\begin{itemize}
\item list, qui correspond à la liste copmlète des combinaisons possibles
\item comp, le comparateur défini juste avant
\end{itemize}
\par
Cette fonction utilise une fonction auxiliare aux prenant en paramètre une liste et le résultat des précédentes éxécutions. Elle consiste à tester chaque élément  de la liste avec le comparateur et ensuite les rajouter ou non dans la liste res.
\section{Elagage}
%\moncode{elag.ml} 
\par
